...
const ProjectDetails = () => {
  ...
  )?.user?._id === project?.members?.find(m => m.role === 'admin')?.user?._id;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          ...
          {showStatusMenu && (
            <div className="absolute right-0 top-12 z-20 bg-gray-800 border border-gray-700 rounded-lg shadow-lg py-1 min-w-[160px]">
              {['pending', 'active', 'completed', 'archived'].map((status) => (
                <button
                  key={status}
        ...
        <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white font-medium">
                {tasksByStatus["Done"].length}/{tasks.length} tasks done
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
        ...
        {insight ? (
          ...
          <p className="text-gray-500">
            Click "Generate" to get AI-powered insights about your project.
          </p>
        )}
      </div>
                    ...
                    <div className="relative">
                      ...
                      {openTaskMenu === task._id && (
                        <>
                          ...
                          />
                          <div className="absolute right-0 top-6 w-40 bg-gray-700 rounded-lg shadow-xl z-20 py-1">
                            <button
                              onClick={() => {
                                setEditingTask(task);
                                setTaskForm({
                              ...
                              className="flex items-center gap-2 w-full px-3 py-2 text-gray-300 hover:bg-gray-600"
                            >
                              <Edit className="w-4 h-4" />
                              Edit
                            </button>
                            <button
                              ...
                              className="flex items-center gap-2 w-full px-3 py-2 text-red-400 hover:bg-gray-600"
                            >
                      ...
                      <div className="flex -space-x-1">
                        {task.assignees.slice(0, 4).map((assignee, idx) => (
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleUserClick(assignee);
                            }}
                            className="w-6 h-6 bg-gradient-to-br from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white text-[10px] font-medium border-2 border-gray-800 hover:scale-110 transition-transform cursor-pointer"
                        ...
                        {task.assignees.length > 4 && (
                          <div className="w-6 h-6 bg-gray-600 rounded-full flex items-center justify-center text-white text-[10px] font-medium border-2 border-gray-800">
                            +{task.assignees.length - 4}
                          </div>
                        )}
                      </div>
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-gray-700">
                    ...
                    {status !== "In Progress" && (
                      <button
                        ...
                        className="flex-1 py-1.5 text-xs bg-blue-600/20 hover:bg-blue-600/30 text-blue-400 rounded transition-colors"
                      >
                        In Progress
                      </button>
                    )}
                    {status !== "Done" && (
                      <button
                        ...
                        className="flex-1 py-1.5 text-xs bg-green-600/20 hover:bg-green-600/30 text-green-400 rounded transition-colors"
                      >
                        Done
                      </button>
                    )}
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
        ...
        <div className="divide-y divide-gray-700">
          {project.members?.map((member, index) => (
            <div
              key={index}
              className="flex items-center gap-4 py-3 px-2 hover:bg-gray-700/50 rounded-lg transition-colors relative"
            >
              <div 
                onClick={() => handleUserClick(member.user, member.role)}
              ...
              >
                ...
                <div className="flex-1">
                  ...
                  <p className="text-gray-400 text-sm">
                    @{member.user?.username || "unknown"} • {member.user?.email || "No email"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                  ...
                  {member.role === 'admin' ? 'Admin' : 
                   member.role === 'project_admin' ? 'Project Admin' : 'Member'}
                </span>
                
                {/* Action menu for admins - can't modify self or the owner */}
                {project.owner === project.members?.find(m => m.role === 'admin')?.user?._id && 
                 member.user?._id !== project.owner && (
                  <div className="relative">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenMemberMenu(openMemberMenu === member.user?._id ? null : member.user?._id);
                      }}
            ...
            <div className="space-y-2">
              {project.members?.map((member) => (
                <label
                  ...
                  className="flex items-center gap-3 p-2 bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-600 transition-colors"
                >
                  <input
                    type="checkbox"
                    checked={taskForm.assignees.includes(member.user?._id || member.user)}
                    onChange={(e) => {
                      const memberId = member.user?._id || member.user;
                      if (e.target.checked) {
                        setTaskForm({
                          ...taskForm,
                          assignees: [...taskForm.assignees, memberId],
                        });
                      } else {
                        setTaskForm({
                          ...taskForm,
                          assignees: taskForm.assignees.filter((id) => id !== memberId),
                        });
                      }
                    }}
        ...
        {selectedUser && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-gray-400">@{selectedUser.username || "unknown"}</p>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 bg-gray-700/50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-white">{selectedUser.email || "No email available"}</p>
                </div>
              </div>
              ...
              {selectedUser.role && (
                <div className="flex items-center gap-3 p-3 bg-gray-700/50 rounded-lg">
                  ...
                  <div>
                    <p className="text-gray-400 text-xs">Role in Project</p>
                    <span className={`inline-block mt-1 px-2 py-1 text-xs font-medium rounded-full ${
                      selectedUser.role === 'admin' 
                        ? 'bg-purple-500/20 text-purple-400 border border-purple-500/30'
                        : selectedUser.role === 'project_admin'
                        ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30'
                        : 'bg-gray-600 text-gray-300'
                    }`}>
                      {selectedUser.role === 'admin' ? 'Admin' : 
                       selectedUser.role === 'project_admin' ? 'Project Admin' : 'Member'}
                    </span>
                  </div>
                </div>
              )}
        ...
        {selectedMember && (
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 p-4 bg-gray-700/50 rounded-lg">
              ...
              <div>
                <p className="text-white font-medium">
                  {selectedMember.user?.fullName || selectedMember.user?.username || "User"}
                </p>
                <p className="text-gray-400 text-sm">@{selectedMember.user?.username || "unknown"}</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {[
                  { value: 'admin', label: 'Admin', desc: 'Full project access and management' },
                  { value: 'project_admin', label: 'Project Admin', desc: 'Can manage tasks and members' },
                  { value: 'member', label: 'Member', desc: 'Can view and work on assigned tasks' }
                ].map((role) => (
                  <button
                    key={role.value}
                    onClick={() => handleUpdateMemberRole(selectedMember.user?._id, role.value)}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <p className={`font-medium ${
                          selectedMember.role === role.value ? 'text-purple-400' : 'text-white'
                        }`}>
                          {role.label}
                        </p>
